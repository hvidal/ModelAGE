...
#ifndef PontaTriangularH

class PontaTriangular: public ParteConector
{
	public:
        PontaTriangular(): ParteConector()
        {
        }
        ~PontaTriangular()
        {
...
void PontaTriangular::Paint(TCanvas *Board, TPoint Origem, TPoint Destino)
{
    ...
    PontoMeio.x = p[0].x + 11 * cos(((Angulo)*M_PI)/180.0);
	PontoMeio.y = p[0].y - 11 * sin(((Angulo)*M_PI)/180.0);
